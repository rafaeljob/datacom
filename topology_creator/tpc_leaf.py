			...
			for j in range(0, len(devices)):
				if i != j: 
					...
					if devices[j] not in host_list:
						...
						if devices[j].get_function() == 'leaf':
							...
							devices[i].append_interface(Interface(local_interface=li_leaf, remote_interface=li_leaf, remote_device=devices[j].get_device_name(),
																	local_ip=local_ip, remote_ip=remote_ip, remote_as=devices[j].get_as_number(), interface_type="x"))	#visao da leaf

							devices[j].append_interface(Interface(local_interface=li_leaf, remote_interface=li_leaf, remote_device=devices[i].get_device_name(),
																	local_ip=remote_ip, remote_ip=local_ip, remote_as=devices[i].get_as_number(), interface_type="x"))	#visao da spine

							leaf_spine+= 1
							spine_local_network_ip+= 100
							host_list.append(devices[i])
